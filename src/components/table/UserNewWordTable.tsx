    ...
    const handlePlaySound = async (item, index) => {
        ...
        arrayMeaning.map(data => {
            ...
            u.voice = voices.find(voice => {
                return voice.name === (data.key === 'en-US' ? enVoiceFromHomePage : vnVoiceFromHomePage)
            })!
            u.rate = 0.8
            synth.speak(u)
        })
        ...
        <div className="flex flex-col w-full bg-white shadow rounded">
            ...
            {tableData.map((item, index) => (
                <div className="flex items-center min-h-[40px] border-b last:border-b-0" key={item[keyId]}>
                    ...
                    <div
                        className="w-[20%] text-center break-all flex items-center gap-2">{item[keyName]}
                        <div className="bg-[#066cfa] w-min rounded-3xl p-2 cursor-pointer"
